...
export default function RepeatedModal({ onClose }: RepeatedModalProps) {
  ...
  const repeatedItems = useMemo(() => {
    ...
    return TEAMS
      .map((team) => {
        ...
        const repeated: { sticker: number; count: number }[] = [];

        stickers.forEach((count, idx) => {
          if (count >= 2) {
            repeated.push({ sticker: idx + 1, count });
          }
        });
  ...
  const totalRepeated = useMemo(() => {
    return repeatedItems.reduce((acc, team) => acc + team.repeated.length, 0);
  }, [repeatedItems]);

  // Group items by group letter
  const groupedItems = useMemo(() => {
    const groups: Record<string, typeof repeatedItems> = {};
    repeatedItems.forEach((team) => {
    ...
    <div className={styles.overlay} onClick={onClose}>
      <div className={styles.modal} onClick={(e) => e.stopPropagation()}>
        <div className={styles.header}>
          <h2 className={styles.title}>Repetidas</h2>
        ...
        <div className={styles.content}>
          <div className={styles.totalBadge}>
            <span className={styles.totalNumber}>{totalRepeated}</span>
            <span className={styles.totalLabel}>láminas repetidas</span>
          </div>
                  ...
                  {teams.map((team) => (
                    <div key={team.code} className={styles.teamCard}>
                      <div className={styles.teamHeader}>
                        ...
                        <span className={styles.teamCount}>{team.repeated.length}</span>
                      </div>
                      <div className={styles.stickerGrid}>
                        {team.repeated.map(({ sticker, count }) => (
                          <div key={sticker} className={styles.stickerCell}>
                            <span className={styles.stickerNum}>{sticker}</span>
